...
function ListingDetailsPage() {
  ...
  const [activeTab, setActiveTab] = useState('comments'); // 'details', 'comments', or 'offer'
  ...
  const handleMarkAs = async (status) => {
    try {
      ...
      setListing(prev => ({ ...prev, marked: status }));
  ...
  const isBargainingAllowed = listing?.type !== 'EXCHANGE' && (listing?.sellListing?.isBargaining || listing?.rentListing?.isBargaining);
    ...
    <div className="min-h-screen bg-white md:bg-gray-50 text-black font-sans selection:bg-black selection:text-white">
      ...
      <main className="px-4 sm:px-12 lg:px-20 py-6 max-w-[1600px] mx-auto">
        ...
        <div className="lg:hidden mb-4 flex gap-2 overflow-x-auto pb-2">
          ...
          {!isOwner && listing.marked === 'PENDING' && listing.type !== 'EXCHANGE' && (
            <>
              {isBargainingAllowed && (
                <button
        ...
        <div className="grid grid-cols-1 lg:grid-cols-9 gap-6">
          ...
          <div className={`lg:col-span-4 bg-white md:border-2 md:border-gray-200 md:rounded-2xl overflow-hidden flex flex-col lg:h-[calc(100vh-120px)] ${
              ...
              <div className="grid grid-cols-2 gap-4 mb-3">
                ...
                <div>
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center gap-2">
                    <p className="text-sm font-bold text-gray-700 break-all">{listing.email}</p>
                    {!isOwner && (
                      <button
                        onClick={handleMessageSeller}
                        className="flex items-center justify-center gap-1 bg-white border border-black text-black px-3 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest hover:bg-gray-50 transition-all whitespace-nowrap flex-shrink-0"
                      >
          ...
          <div className={`lg:col-span-5 bg-white md:border-2 md:border-gray-200 md:rounded-2xl overflow-hidden flex flex-col lg:h-[calc(100vh-120px)] ${
            activeTab === 'details' ? 'hidden lg:flex' : ''
          }`}>
            {/* Desktop Tab Navigation - Only show on desktop and not for EXCHANGE type */}
            {listing.type !== 'EXCHANGE' && (
              <div className="hidden lg:flex border-b-2 border-gray-200 bg-gray-50">
                <button
                  onClick={() => setActiveTab('comments')}
                  className={`flex-1 py-3 px-4 text-sm font-black uppercase tracking-wider transition-all ${
                    activeTab === 'comments'
                      ? 'bg-white text-black border-b-2 border-black -mb-[2px]'
                      : 'text-gray-500 hover:text-gray-700'
                  }`}
                >
                  Comments
                </button>
                {!isOwner && listing.marked === 'PENDING' && isBargainingAllowed && (
                  <button
                    onClick={() => setActiveTab('offer')}
                    className={`flex-1 py-3 px-4 text-sm font-black uppercase tracking-wider transition-all ${
                      activeTab === 'offer'
            ...
            {activeTab === 'comments' || listing.type === 'EXCHANGE' ? (
              <div className="flex-1 overflow-hidden">
                <CommentSection listingId={id} />
              </div>
            ) : activeTab === 'offer' && listing.type !== 'EXCHANGE' && isBargainingAllowed ? (
              <>
